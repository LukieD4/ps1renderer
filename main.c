...
extern const unsigned char tex_blob_start[];
extern const unsigned char tex_blob_end[];
...
extern const unsigned int texBlobEntryCount;
extern const TEX_BLOB_ENTRY texBlobMaterialTable[];
...
void load_textures(void)
{
    ...
    for (unsigned int i = 0; i < texBlobEntryCount && textureTableCount < MAX_TEXTURES; i++)
    {
        ...
        // Upload CLUT data to VRAM at the rect the TIM specifies. 4-bit
        // TIMs always carry a CLUT (16 colors); this would be skipped for
        // a 24-bit direct-color TIM, but we're 4-bit here so crect/caddr
        // are always set for every entry in this blob.
        if (slot->tim.crect != 0)
            LoadImage(slot->tim.crect, slot->tim.caddr);
...
// entry in textureTable[] (flat-color materials like Garage/House0) get
...
void draw_model(void)
{
    ...
    for (unsigned int order = 0; order < triCount; order++)
    {
        ...
        if (use_texture && tex != NULL)
        {
            ...
            setClut(&polyFT, tex->tim.crect->x, tex->tim.crect->y);
