...
(function (root) {
  ...
  function encode(pcm, options) {
    ...
    for (var b = 0; b < realBlocks; b++) {
      ...
      var flag = FLAG_NONE;
      if (loop) {
        // OR the flags together: if the loop starts on the final block the
        // combined value is 0x07 (start + end + repeat), not a bare 0x03
        // that would lose the start marker.
