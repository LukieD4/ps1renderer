...
# ------------------------------------------------------------------
# Stage discovery. A stage is identified by its .json FILE, and the file's
# NAME is the stage name: stage-gen now exports "<stage name>.json" (it used
# to export a fixed "stage.json", where the stage was named after the
# CONTAINING FOLDER instead). Both on-disk layouts are accepted, so an
# artist can drop the exported file straight in or keep a folder per stage:
#
#     assets/stage/House.json          - flat, one file per stage
#     assets/stage/House/House.json    - a folder per stage
#
# and legacy assets/stage/<dir>/stage.json still converts, keeping the old
# behaviour of naming the stage after the FOLDER - a file literally called
# "stage" is the old fixed filename, not an authored stage name.
# ------------------------------------------------------------------
def discover_stage_files(root):
    if not root.exists():
        return []
    seen = set()
    found = []
    # Flat (*.json) and one-level-deep (*/*.json), de-duped by resolved path
    # and sorted for stable, reproducible generated output.
    for p in sorted(set(root.glob("*.json")) | set(root.glob("*/*.json"))):
        rp = p.resolve()
        if rp in seen:
            continue
        seen.add(rp)
...
def stage_name_from_path(path):
    # Legacy fixed filename ("stage.json"): name the stage after its folder,
    # exactly as before. Anything else: the filename, minus the .json
    # extension, IS the stage name.
    if path.stem == "stage":
        return path.parent.name
    return path.stem


stage_paths = discover_stage_files(STAGES_DIR)

if not stage_paths:
    print("No stage .json files found under assets/stage/ - nothing to do.")

stage_entries = []  # dicts: name, ident, cam_pos, cam_rot (object arrays written to disk immediately below)

# ident -> source path, so two stages that sanitize to the same C identifier
# are caught here rather than silently clobbering each other's generated file.
seen_idents = {}

for stage_path in stage_paths:
    stage_name = stage_name_from_path(stage_path)
    ident = sanitize_identifier(stage_name)

    # Two stage files that reduce to the same C identifier would both write
    # stage_<ident>.c and emit identically-named array sets - the second
    # clobbering the first on disk and colliding at link time. Stop here,
    # where the fix (rename one stage's .json) is obvious, instead of shipping
    # a corrupt build. Note filenames differing only in case, or in a
    # character sanitize_identifier() flattens to '_', can trip this.
    if ident in seen_idents:
        raise SystemExit(
            f"{stage_path}: stage name {stage_name!r} collides with "
            f"{seen_idents[ident]} - both map to C identifier '{ident}'. "
            f"Rename one stage's .json file."
        )
    seen_idents[ident] = stage_path
...
# ------------------------------------------------------------------
# Registry - one entry per discovered stage .json (see discover_stage_files).
# ------------------------------------------------------------------
registry_lines = []
